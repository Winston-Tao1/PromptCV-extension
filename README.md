...
**强大的Chrome浏览器扩展，集提示词管理、云盘存储、富文本编辑于一体**
...
- 🗑️ **快速删除** - 缩略图一键删除
- 💾 **自动保存** - 2秒无操作自动保存，无需手动
- ☁️ **云端同步** - 通过Google账号跨设备同步

### 🎨 个性化定制
- 🖼️ **自定义Logo** - 上传个人Logo，打造专属品牌（最大1MB）
- ✏️ **可编辑标题** - 自定义扩展名称（最多20字符）
- 🌈 **Google风格** - Material Design设计语言
- 💫 **流畅动画** - 精心设计的交互反馈

### 💾 数据管理
- 🔄 **云端同步** - Chrome Storage Sync API，跨设备访问
- 🔒 **数据安全** - 本地存储 + 云端备份
- 📊 **智能限制** - 自动维护20条记录，优化性能
- ⚡ **即时响应** - 轻量级设计，毫秒级响应
...
#### 上传文件
1. 点击绿色"**📤 上传文件**"按钮
2. 选择支持的文件格式（txt、doc、pdf、xls、ppt等）
3. 文件以缩略图形式显示在光标位置
4. 点击缩略图上的 **×** 可删除文件
...
#### 保存和同步
- **自动保存**：输入停止2秒后自动保存
- **手动保存**：点击"**保存**"按钮立即保存并同步到云端
- **跨设备访问**：登录Chrome账号后自动同步到所有设备
...
├── CLOUD_DISK_FEATURE.md      # 云盘功能详细文档
├── icons/                     # 图标资源文件夹
...
| Chrome Storage API | Sync + Local | 数据持久化与同步 |
...
| 云盘内容 | Chrome Sync | ✅ | 100KB |
| 自定义Logo | Chrome Local | ❌ | 1MB |
| 扩展标题 | Chrome Sync | ✅ | 20字符 |
...
1. 检查是否登录Chrome账号
2. 确认Chrome同步功能已开启
3. 查看浏览器设置 → 同步和Google服务
4. 尝试在其他设备上查看（数据可能已同步到云端）
...
#### Q4: 云盘文件上传没有反应❓
**A:**
1. 确认文件格式是否支持（txt、doc、docx、pdf、xls、xlsx、ppt、pptx）
2. 检查文件是否过大（建议 < 1MB）
3. 尝试使用其他文件测试
4. 查看浏览器控制台是否有错误信息
